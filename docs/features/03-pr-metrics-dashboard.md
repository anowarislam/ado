...
Location: Repository root (`/Users/anowarislam/Projects/ado/.testcoverage.yml`)
  ...
  go:
    ...
    steps:
      ...
      - name: Calculate estimated cost
        ...
        id: cost
        run: |
          # Get workflow start time from GitHub context
          ...
          DURATION_SECONDS=$((CURRENT_TIME - START_EPOCH))
          ...
          echo "" >> $GITHUB_STEP_SUMMARY
          echo "> **Note:** This is an estimate based on workflow duration. Actual billed minutes may differ." >> $GITHUB_STEP_SUMMARY
...
| Fork PR from external contributor | PR comment requires `pull-requests: write` permission | Use `pull_request_target` event or configure permissions appropriately |
...
- [ ] Pin GitHub Actions to SHA (security requirement)
